...
def admin_panel():
    ...
    try:
        # ✅ FILTRUJ KAMPANĚ PODLE USER_ID
        campaigns = cold_db.get_campaigns(user_id=user_id)
...
def admin_create_campaign():
    ...
    user_id = session['user_id']  # ✅ PŘIDEJ USER_ID
    ...
    try:
        campaign_id = cold_db.create_campaign(name, description, user_id=user_id)  # ✅ POŠLI USER_ID
...
def admin_campaign(campaign_id):
    ...
    try:
        # ✅ KONTROLA BEZPEČNOSTI - Je to kampaň uživatele?
        conn = sqlite3.connect("data/cold_calling.db")
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM campaigns WHERE id = ? AND (user_id = ? OR user_id IS NULL)", (campaign_id, user_id))
        campaign = cursor.fetchone()
        conn.close()
        
        if not campaign:
            flash('Nemáte přístup k této kampani', 'error')
            return redirect('/admin')
...
# RESET kontaktů v kampani
@app.route('/admin/reset/<int:campaign_id>', methods=['GET'])
@login_required
def admin_reset_contacts(campaign_id):
    """Smaže všechny kontakty v této kampani"""
    cold_db.delete_contacts_in_campaign(campaign_id)  # Implementuj viz níže!
    flash('Všechny kontakty kampaně byly smazány!', 'success')
    return redirect(f'/admin/campaign/{campaign_id}')

# SMAZÁNÍ CELÉ KAMPANĚ
@app.route('/admin/delete-campaign/<int:campaign_id>', methods=['GET'])
@login_required
def admin_delete_campaign(campaign_id):
    """Smaže kampaň včetně kontaktů + možnost opětovného použití ID"""
    cold_db.delete_contacts_in_campaign(campaign_id)  # Smaž nejdřív kontakty!
    cold_db.delete_campaign(campaign_id)              # Smaž kampaň!
    flash('Kampaň včetně kontaktů byla smazána!', 'success')
    return redirect('/admin')
# ============================================================
...
def process_speech():
    ...
    if not user_input or len(user_input.strip()) < 2:
        ...
        # ✅ Vyberi zprávu
        if retry_count == 0:
            retry_message = "Slyšíte mě?"
        else:
            retry_message = "Zopakujte prosím, neslyším vás dobře."
        
        print(f"  ℹ️  Retry {retry_count + 1}: {retry_message}")
        
        # ✅ NOVÉ: Vygeneruj audio přes TTS (nie Twilio voice!)
        try:
            audio_url = tts.generate(retry_message, use_cache=True)
            ...
            # Přehraj audio (nikoliv Twilio say!)
            gather.play(audio_url)
            response.append(gather)
            response.redirect(f'/process?retry={retry_count + 1}&call_time={call_time + 4}')
            
        except:
            # Fallback: Použij Twilio voice jen když TTS selže
            gather = Gather(
                input='speech',
                action=f'/process?retry={retry_count + 1}&call_time={call_time + 4}',
                language='cs-CZ',
            ...
            gather.say(retry_message, language='cs-CZ', voice='woman')
            response.append(gather)
            response.redirect(f'/process?retry={retry_count + 1}&call_time={call_time + 4}')
